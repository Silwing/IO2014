...
void test(IN* in, OUT* out, int k, int n, int b, string testName) {
    ...
    results << testName << "\t" << n << "\t" << k << "\t" << b << "\t" << std::chrono::duration_cast<std::chrono::milliseconds>(t2-t1).count();
    for (int l=0; l<k; l++) {
        out[l]->close();
        delete out[l];
        in[l]->open();
